...
from assistant.core.exceptions import AudioDeviceError
...
class AudioDeviceCatalog:
    ...
    def list_input_devices(self) -> list[AudioDevice]:
        return [device for device in self._all_devices() if device.input_channels > 0]

    def list_output_devices(self) -> list[AudioDevice]:
        return [device for device in self._all_devices() if device.output_channels > 0]
    ...
    def get_device(self, index: int) -> AudioDevice:
        device = next((item for item in self._all_devices() if item.index == index), None)
        if device is None:
    ...
    def _all_devices(self) -> list[AudioDevice]:
    ...
    @staticmethod
    def _find_device(index: int | None, devices: list[AudioDevice]) -> AudioDevice | None:
        if index is None:
            return None
